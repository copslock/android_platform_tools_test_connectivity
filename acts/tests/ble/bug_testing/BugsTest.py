...
class BugsTest(BaseTestClass):
  ...
  def blescan_verify_onscanresult_event_handler(self, event,
    ...
    if callbacktype != expected_callbacktype:
      ...
      test_result = False
    self.log.debug("meow: " + event['data']['Result']['deviceInfo']['address'])
    return test_result
  ...
  def test_three_advertisers_and_three_scanners(self):
    ...
    test_result = startbleadvertise(advertise_droid, advertise_data,
                                    advertise_settings,
                                    advertise_callback)
    test_result = startbleadvertise(advertise_droid, advertise_data1,
                                    advertise_settings1,
                                    advertise_callback1)
    test_result = startbleadvertise(advertise_droid, advertise_data2,
                                    advertise_settings2,
                                    advertise_callback2)

    test_result = startblescan(scan_droid, filter_list, scan_settings,
                               scan_callback)
    test_result = startblescan(scan_droid, filter_list1, scan_settings1,
                               scan_callback1)
    test_result = startblescan(scan_droid, filter_list2, scan_settings2,
                               scan_callback2)
    time.sleep(30)
    self.log.debug(
      "Step 5: Verify the Bluetooth Le Scan did not cause an onScanFailed event.")
